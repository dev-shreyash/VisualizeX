...
export const HoverEffect = ({
  ...
  const router = useRouter(); // Fix: Use router for navigation
    ...
    >
      {items.map((item, idx) => (
        <div
          ...
          onClick={() => router.push(item.link)} // Fix: Navigates correctly
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
...
export const Card = ({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl h-full w-full p-4 shadow-md overflow-hidden bg-slate-200 border border-transparent dark:border-white/[0.2] group-hover:border-slate-700 relative z-20",
        className
      )}
    >
